...
提示されたPCAPキャプチャ解析サマリを元に、Tier 1 / Tier 2 アナリスト向けの実践的な選択式クイズを {num_questions} 問作成してください。

【作成指針】
- 問題文、選択肢、および解説はすべて「日本語」で記述してください。
- 不審な通信（Malicious IP, C2, DNS Tunneling, Web攻撃、異常なポート使用等）や調査観点を問う問題を優先してください。
- 4つの選択肢 (A, B, C, D) のうち、正解 (answer) は 'A', 'B', 'C', 'D' のいずれか1つの文字のみを指定してください。
- 解説 (explanation) には、なぜそれが正解なのか、どのプロトコルやデータに基づくのかを詳しく記載してください。
