...
## [2026-09-15] - v1.8.0
### 変更・改善概要
出力ファイル構造および CLI 出力オプションの刷新。1回の実行でトレーサビリティ（再現性・デバッグ性）を確保するため、PCAPファイル名ベースのディレクトリへ5つの関連ファイル（HTML, Aiken, パケット要約, プロンプト, 生JSON）を一括出力する構造へ拡張。

### 追加・変更点
1. **ディレクトリ作成および5ファイルの自動一括出力**
   - デフォルトで PCAP ファイル名（拡張子除く）のディレクトリを作成し、以下のファイルを出力するよう改修:
     - `<PCAPベース名>.html`: インタラクティブHTML形式クイズ
     - `<PCAPベース名>_aiken.txt`: Moodle用Aiken形式クイズ
     - `summary.txt`: パケット解析要約テキスト
     - `prompt.txt`: LLMに与えた入力プロンプトテキスト
     - `response.json`: LLMから返された生JSONデータ
2. **CLI 出力フォーマット指定オプションの改修 (`pcap2quiz.py`)**
   - 単一ファイル指定の `--format` (`-f`) オプションを廃止。
   - ディレクトリパス指定の `-o, --output` オプションへ変更。
   - クイズ出力形式の制御用フラグ `--html-only` および `--aiken-only` を追加（未指定時は両方出力）。
3. **ドキュメント類の同期更新 (`AGENTS.md`, `README.md`)**
   - `AGENTS.md` および `README.md` の使用法・出力仕様・CLIオプション一覧を新しいディレクトリ出力仕様に更新。

## [2026-09-15] - v1.9.0
### 変更・改善概要
Python パッケージ（CLI ツール）化に対応し、`pip install .` や `pip install -e .` による環境へのインストールを可能に改修。`site-packages` 直下での名前衝突を防ぎ、アセットやテンプレートを安全に参照できるようパッケージ専用ディレクトリ構造（`pcap2quiz/`）へリファクタリング。

### 追加・変更点
1. **`pyproject.toml` の作成**
   - PEP 621 形式のビルド設定を定義し、CLI エントリーポイント `pcap2quiz = pcap2quiz.cli:main` を登録。
2. **専用パッケージ構造（`pcap2quiz/`）への移行 (`git mv`)**
   - 変更履歴を維持したまま `git mv` で全ファイル・モジュールを `pcap2quiz/` ディレクトリ配下に再配置。
   - `pcap2quiz/__init__.py` (バージョン `1.9.0`) および `pcap2quiz/__main__.py` を新設。
3. **インポート文の更新**
   - パッケージ化に合わせて内部モジュールのインポートパスを `pcap2quiz.parsers` 等へ修正。
