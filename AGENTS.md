...
- **標準対応プロトコル・抽出機能 (`parsers/`):**
  ...
  - `SMTPExtractor`: SMTP コマンド、レスポンス、送信元/送信先メールアドレス等
  - `POP3Extractor`: POP3 コマンド、レスポンス、ログイン試行ユーザー名等
  - `IMAPExtractor`: IMAP コマンド、レスポンス、ログイン試行ユーザー名等
- **拡張性 (開閉原則):**
...
- Ollama API 呼び出し
- プロンプトテンプレート:
  - 外部テンプレートファイル (`templates/quiz_prompt.txt`) を読み込み、要約テキストおよびパラメータを埋め込んで生成。
